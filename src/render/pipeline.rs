...
impl Pipeline {
    ...
    pub fn redraw(
        ...
        // Run renderpass (with egui at the end).
        {
            ...
            // Run shader, clipped to the area not covered by UI panels.
            if let Some(bind_groups) = bind_groups {
                let scale = self.egui_screen_descriptor.pixels_per_point;
                let x = (visible_rect.min.x * scale) as u32;
                let y = (visible_rect.min.y * scale) as u32;
                let w = (visible_rect.width() * scale) as u32;
                let h = (visible_rect.height() * scale) as u32;
                let [sw, sh] = self.egui_screen_descriptor.size_in_pixels;
                if w > 0 && h > 0 && x + w <= sw && y + h <= sh {
